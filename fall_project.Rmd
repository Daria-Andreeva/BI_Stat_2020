...
Расчеты проведены в среде R с использованием пакета pwr [10]. Для построения графиков использовали пакет ggplot2 [11].
...
mol_data <- read_files('/home/daria/Bioinf/statistics_r/data/Data/')
```
...
box_rings <- ggplot(new_mol, aes(y = Rings)) +  #box plot
  geom_boxplot() +
theme_classic()
plot_rings <- grid.arrange(qq_rings, density_rings,
                           box_rings, ncol = 2)
```
The average value of the Rings parameter is 10, the mode and median are 9. The shape of the distribution is asymmetric, unimodal, the distribution has a slight positive skew. However, since the mode and median of this distribution coincide, and the median does not differ much, the distribution is very close to normal.
...
plot_Length <- grid.arrange(qq_Length, density_Length,
                           box_Length, ncol = 2)
```
The mean of the Length parameter is 0.524, the mode is 0.550 and the median is 0.545. The shape of the distribution is asymmetric, unimodal, the distribution has a slight negative skew.
...
plot_Diameter <- grid.arrange(qq_Diameter, density_Diameter,
                           box_Diameter, ncol = 2)
```
The mean of the Diameter parameter is 0.408, the mode is 0.450 and the median is 0.425. The shape of the distribution is asymmetric, unimodal, the distribution has a slight negative skew. It is chi-square distribution. 
...
plot_Height <- grid.arrange(qq_Height, density_Height,
                           box_Height, ncol = 2)
```
The mean of the Height parameter is 0.408, the mode is 0.450 and the median is 0.425. The shape of the distribution is asymmetric, unimodal, the distribution has a slight positive skew, because of two outliers. If we omit them, it would be normal.
...
```
The mean of the Whole weight parameter is 0.829, the mode is 0.223 and the median is 0.801. The shape of the distribution is chi-squired, asymmetric, bimodal.
...
```
The mean of the Whole weight parameter is 0.360, the mode is 0.175 and the median is 0.336. The shape of the distribution is chi-squired, asymmetric, unimodal.
...
```
The mean of the Viscera weight parameter is 0.181, the mode is 0.172 and the median is 0.171. The shape of the distribution is chi-squired, asymmetric, unimodal.
...
```
The mean of the Shell weight parameter is 0.239, the mode is 0.275 and the median is 0.171. The shape of the distribution is chi-squired, asymmetric, bimodal with positive skew.
...
################################################### Я не знаю, что еще можно сделать с графиком, чтобы он нормально показывался, не открывая его дополнительно в окне :(((((((((((((((((((((
```
...
We see very strong correlation between Diameter and Whole_weight (0.93), p < 0.01. 
```{r}
